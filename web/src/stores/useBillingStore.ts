...
interface BillingState {
    ...
    usage: { query_count: number; project_count: number; storage_bytes: number };
    loading: boolean;
...
export const useBillingStore = create<BillingState>((set) => ({
    tier: 'free',
    limits: { queries: 50, projects: 3, storage_gb: 0.1 }, 
    usage: { query_count: 0, project_count: 0, storage_bytes: 0 },
    loading: true,
    fetchBilling: async () => {
        try {
