...
MANIFEST_FILES = {
    ...
    'go.mod', 'dockerfile', 'docker-compose.yml', 'makefile', 'pom.xml',
    'build.gradle'
}
...
class AdvancedCodeParser:
    ...
    def filter_process(self, file_path: str) -> bool:
        ...
        if file_name in MANIFEST_FILES or file_name.endswith('.md'):
            return True # Always allow manifests and markdown
    ...
    def _get_language(self, file_path: str) -> Optional[str]:
        ...
        mapping = {
            ...
            "java": "java", "cpp": "cpp", "cc": "cpp", "cxx": "cpp", "h": "cpp", "hpp": "cpp",
            "c": "c", "cs": "c_sharp", "rb": "ruby", "php": "php",
        }
    ...
    def parse_file(self, file_path: str, content: bytes = None) -> List[CodeBlock]:
        ...
        # 2. Check for Markdown or Manifest files
        file_name = os.path.basename(file_path).lower()
        _, ext = os.path.splitext(file_path)
        
        if ext.lower() == ".md" or file_name in MANIFEST_FILES:
            print(f"📄 Non-AST config/doc file detected: {file_name}")
            ...
            return [CodeBlock(
                ...
                identifier=f"{os.path.relpath(file_path)}::root::{original_file_name}",
                parent_block=None
            )]
    ...
    def _process_def_node(self, node, content: bytes, file_path: str, parent_scope: str, lang: str) -> Optional[CodeBlock]:
        ...
        return CodeBlock(
            name=name, type=b_type, content=body_str,
            start_line=node.start_point.row, end_line=node.end_point.row,
            file_path=os.path.relpath(file_path), identifier=identifier,
            parent_block=parent_scope, calls=calls, bases=bases
        )
